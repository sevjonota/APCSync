window.api = (() => {
    ...
    function isPastDateTime(dateValue, timeValue) {
        if (typeof dateValue !== 'string' || typeof timeValue !== 'string' || !dateValue || !timeValue) {
            return false;
        }
    ...
    async function createBooking(payload) {
        return runWithDelay(() => {
            ...
            if (isPastDateTime(date, startTime)) {
                throw createApiError('PAST_SLOT_ERROR', 'Room reservations must be scheduled for a future date and time', {
                    date: [`Reservation slot (${date} ${startTime}) has already started or passed`]
                }, 422);
            }
