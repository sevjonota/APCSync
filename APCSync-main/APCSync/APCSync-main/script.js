...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const ramseyLogoPath = '../png/Ramsey.png';
    ...
    function clearAssistantConversationView() {
        if (chatMessagesWrapper) chatMessagesWrapper.innerHTML = '';
    ...
    function renderAssistantConversation() {
        ...
        const conversation = assistantService ? assistantService.getAssistantConversation(getCurrentUserKey()) : [];
        ...
        conversation.forEach((entry) => {
            ...
            messageEl.className = `chat-message ${isAssistant ? 'left' : 'right'}`;
            messageEl.innerHTML = isAssistant
                ? `
                    <div class="bot-icon">${getRamseyAvatarMarkup()}</div>
                    <div class="message-bubble bot">${messageText}</div>
                `
                : `
                    <div class="message-bubble user">${messageText}</div>
                    <div class="user-icon"><i class="fas fa-user"></i></div>
                `;
            chatMessagesWrapper.appendChild(messageEl);
        });

        if (!chatMessagesWrapper.children.length) {
            chatEmptyState.classList.remove('hidden-state');
        }
    }
    ...
    function renderDefaultPhotoOptions() {
        ...
        eventDefaultPhotosContainer.innerHTML = defaultEventPhotos.map(photo => {
            return `<button type="button" class="default-photo-option" data-key="${photo.key}" data-name="${photo.name}"><img src="${photo.src}" alt="${photo.name}"></button>`;
        }).join('');
    ...
    async function updateRoomLockingForBooking() {
        ...
        try {
            ...
            if (matchingBookings.length > 0) {
                ...
                const bookingDate = booking.date || booking.booking_date || date;
                const bookingStart = booking.startTime || booking.start_time || startTime;
                const bookingEnd = booking.endTime || booking.end_time || endTime;
                
                dateInput.value = bookingDate;
                startTimeInput.value = bookingStart;
                endTimeInput.value = bookingEnd;
    ...
    async function renderDashboard(eventsByDate = null) {
        ...
        if (invitationsContainer) {
            ...
            console.log(`[DEBUG] Rendering invitations, found ${userInvitations.length} invitations`);
                ...
                invitationsContainer.innerHTML = userInvitations.map(invitation => {
                    ...
                    return `
                        <div class="card" style="margin-bottom:1rem; cursor:pointer; overflow:hidden; position:relative; ${bgStyle}" data-invitation-id="${invitation.id}">
                            <div style="position:absolute; top:0; left:0; right:0; bottom:0; background:rgba(255,255,255,0.98); backdrop-filter:blur(2px); padding:1rem; display:flex; justify-content:space-between; align-items:center; gap:1.5rem;">
    ...
    // Handle student email invitation
    const eventInviteInput = document.getElementById('event-invite-students');
    if (eventInviteInput) {
        // Define valid student emails
        const validStudentEmails = {
            'someone@student.apc.edu.ph': 'Student User 1',
            'another@student.apc.edu.ph': 'Student User 2'
        };

        eventInviteInput.addEventListener('keypress', (e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                const email = eventInviteInput.value.trim().toLowerCase();
                
                // Validate email
                if (!validStudentEmails[email]) {
                    showNotice('Invalid student email. Use someone@student.apc.edu.ph or another@student.apc.edu.ph', 'error');
                    return;
                }
                ...
                const student = {
                    id: `u-${email}`,
                    email: email,
                    name: validStudentEmails[email]
                };

                // Check if already invited
                const alreadyInvited = invitedUsers.some(u => u.email === email);
                if (alreadyInvited) {
                    showNotice(`${validStudentEmails[email]} is already invited.`, 'error');
                ...
                if (currentUser && currentUser.email && currentUser.email.toLowerCase() === email) {
                    showNotice('You cannot invite yourself.', 'error');
                    eventInviteInput.value = '';
                    return;
                }

                // Add to invited users
                invitedUsers.push(student);
                eventInviteInput.value = '';
                ...
                if (invitedDiv) {
                    ...
                    invitedDiv.querySelectorAll('.btn-remove-invite').forEach(btn => {
                        btn.addEventListener('click', (e) => {
                            ...
                            // Re-render by triggering a dummy event
                            const dummyEvent = new Event('keypress');
                            dummyEvent.key = 'Dummy';
                            eventInviteInput.dispatchEvent(dummyEvent);
                        });
                    });
                }
            }
    ...
    async function saveEventHandler(e) {
        ...
        try {
            ...
            if (ev) {
                ...
                // If user typed an email but didn't press Enter, include it now
                const pendingInputEl = document.getElementById('event-invite-students');
                const pendingEmail = pendingInputEl ? String(pendingInputEl.value || '').trim().toLowerCase() : '';
                if (pendingEmail) {
                    const allowed = {
                        'someone@student.apc.edu.ph': 'Student User',
                        'another@student.apc.edu.ph': 'Student User 2'
                    };
                    if (allowed[pendingEmail]) {
                        const alreadyInvited = invitedUsers.some(u => u.email === pendingEmail);
                        const currentUserEmail = (currentUser && currentUser.email) ? String(currentUser.email).trim().toLowerCase() : '';
                        if (!alreadyInvited && pendingEmail !== currentUserEmail) {
                            invitedUsers.push({ id: `u-${pendingEmail}`, email: pendingEmail, name: allowed[pendingEmail] });
                        }
                        if (pendingInputEl) pendingInputEl.value = '';

                        // Re-render invited users list if present
                        const invitedDiv = document.getElementById('event-invited-users');
                        if (invitedDiv) {
                            invitedDiv.innerHTML = '';
                            ...
                            invitedDiv.querySelectorAll('.btn-remove-invite').forEach(btn => {
                                btn.addEventListener('click', (e) => {
                                    ...
                                    invitedUsers.forEach((user) => {
                                        ...
                                        t.style.cssText = 'display: flex; align-items: center; gap: 0.5rem; padding: 0.5rem 0.75rem; background: var(--blue-50); border: 1px solid var(--blue-200); border-radius: var(--radius-sm); font-size: 0.875rem;';
                                        t.innerHTML = `
                                            <span><strong>${user.name}</strong> (${user.email})</span>
                                            <button type="button" data-user-id="${user.id}" class="btn-remove-invite" style="background: none; border: none; color: var(--text-secondary); cursor: pointer; padding: 0; font-weight: bold;">×</button>
                                        `;
                                        invitedDiv.appendChild(t);
                                    });
                                });
                            });
                        }
                ...
                if (invitedUsers && invitedUsers.length > 0) {
                    console.log(`[DEBUG] Sending ${invitedUsers.length} invitation(s) for event:`, ev.id);
                    invitedUsers.forEach(invitedUser => {
                        console.log(`[DEBUG] Inviting ${invitedUser.email} to event ${ev.title}`);
    ...
    if (chatSuggestionBtns.length > 0 && chatMessagesWrapper) {
        chatSuggestionBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                const assistantService = getAssistantService();
                if (assistantService) {
                    const history = assistantService.getAssistantConversation(getCurrentUserKey());
                    history.push(
                        { role: 'user', message: questionText, questionId, createdAt: new Date().toISOString() },
                        { role: 'assistant', message: answerText, questionId, createdAt: new Date().toISOString() }
                    );
                    assistantService.setAssistantConversation(history, getCurrentUserKey());
                }
